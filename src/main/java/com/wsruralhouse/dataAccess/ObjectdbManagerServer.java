...
public class ObjectdbManagerServer extends JDialog {
	...
	private static final String OBJECTDB_PATH = "src\\main\\java\\resources\\objectdb.jar";
	...
	public ObjectdbManagerServer() {
				...
				okButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						try {
							...
							try {
								Runtime.getRuntime().exec("java -cp " + OBJECTDB_PATH + " com.objectdb.Server -port "+ c.getDatabasePort()+" stop");
							} catch (Exception ioe) {
		...
		if (c.isLocalDatabase()) {
			...
			try{
				...
				try {
					Runtime.getRuntime().exec("java -cp " + OBJECTDB_PATH + " com.objectdb.Server -port "+ c.getDatabasePort()+" start");
				} catch (Exception ioe) {
					System.out.println (ioe);
				}
